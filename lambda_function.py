        ...
        for w in turned_to_dict :
            ...
            if 'REPORT' in w['message']:
                ...
                try:
                    ...
                    mstn = 0
                message = ''
                details = [owner,loggroup,logstream,subscription_filters,timestamp,'START',id_,billed_duration,memory_size,memory_used,Init_Duration,mstn,'']
            #print(details)
                ...
                details = [owner,loggroup,logstream,subscription_filters,timestamp,'END',id_,0,0,0,0,0,message]
            elif 'START' in w['message']:
                id_ = w['id']
                timestamp= str(datetime.datetime.fromtimestamp(int(str(w['timestamp'])[:10]) ))
                message_type = 'START'
                message = w['message'].split(':')[1].replace('Version','')
                details = [owner,loggroup,logstream,subscription_filters,timestamp,'REPORT',id_,0,0,0,0,0,message]
            #print(id_,timestamp,message)
            
            parse_data.append(details)
#             cur.execute(f'''INSERT INTO  TESTT VALUES ('{owner}','{loggroup}','{logstream}','{subscription_filters}','{timestamp}','{message_type}','{id_}','{billed_duration}','{memory_size}','{memory_used}','{Init_Duration}','{mstn}','{message}')'''
# )
